...
def window_results(rows: list[dict]) -> dict[str, dict]:
    ...
    for r in rows:
        ...
        w = out.setdefault(slug, {"coin": slug.split("-", 1)[0].upper(), "cash": 0.0, "bought": 0.0, "shares": 0.0, "settled": False})
...
def main() -> None:
    ...
    per = defaultdict(lambda: {"n": 0, "w": 0, "l": 0, "pnl": 0.0, "staked": 0.0})
    for w in done.values():
        ...
        if w["cash"] > 0:
            ...
            p["l"] += 1
    print(f"{'COIN':6}{'WINDOWS':>9}{'WIN':>6}{'LOSS':>6}{'WIN%':>7}{'P&L':>11}{'STAKED':>10}{'ROI':>8}")
    tot = {"n": 0, "w": 0, "l": 0, "pnl": 0.0, "staked": 0.0}
    for coin in sorted(per):
        ...
        print(f"{coin:6}{p['n']:>9}{p['w']:>6}{p['l']:>6}{wr:>6.0f}%{p['pnl']:>+11.2f}{p['staked']:>10.2f}{roi:>+7.1f}%")
    wr = tot["w"] / (tot["w"] + tot["l"]) * 100 if tot["w"] + tot["l"] else 0
    roi = tot["pnl"] / tot["staked"] * 100 if tot["staked"] else 0
    print(f"{'TOTAL':6}{tot['n']:>9}{tot['w']:>6}{tot['l']:>6}{wr:>6.0f}%{tot['pnl']:>+11.2f}{tot['staked']:>10.2f}{roi:>+7.1f}%")
